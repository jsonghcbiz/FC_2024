...
OUTPUT_PATH = '/data/ephemeral/home/NLP_JES/llm/output'
...
### ✅ 저장할 파일 이름 세팅 ✅
FILE_NAME = f'test_eng_{START_IDX}_{END_IDX}.json'

### ✅ 배치 사이즈 세팅 ✅
BATCH_SIZE = 22     # ⚠️⚠️⚠️ output token 초과 시 데이터 추출이 제대로 안됨. 시간이 걸리더라도 배치 사이즈를 줄여야 함.
...
# 데이터 로드
###############################################
def load_data(file_path=OUTPUT_PATH, start_idx=0, end_idx=None):
    output_df = pd.read_csv(os.path.join(file_path,'test_output_eng.csv'))
    if end_idx is not None:
        output_df = output_df.iloc[start_idx:end_idx]
    else:
        output_df = output_df.iloc[start_idx:]
    # output_df['english_dialogue'] = output_df['english_dialogue'].apply(lambda x: x.replace('\n', ' '))
    return output_df

def load_reference_data(file_path=RESULT_PATH):
    original_df = pd.read_csv(os.path.join(file_path, f"test.csv"))
    reference_df = pd.read_csv(os.path.join(file_path, f"train_dh_v2_eng.csv"))
    return original_df, reference_df

###############################################
# Few-shot 예시 샘플링
###############################################
def few_shot_sample(original_df: pd.DataFrame, reference_df: pd.DataFrame) -> Tuple[List[str], List[str], List[str], List[str], List[str], List[str]]:
    original_samples = original_df[:FS_NUM_SAMPLES]
    reference_samples = reference_df[:FS_NUM_SAMPLES]
    
    original_fname = original_samples['fname'].tolist()
    original_kr_dialogue = original_samples['dialogue'].tolist()
    exp_fname = reference_samples['fname'].tolist()
    exp_kr_dialogue = reference_samples['korean_dialogue'].tolist()
    exp_en_summary = reference_samples['english_summary'].tolist()
    exp_kr_summary = reference_samples['korean_summary'].tolist()
    
    return original_fname, original_kr_dialogue, exp_fname, exp_kr_dialogue, exp_en_summary, exp_kr_summary

def format_few_shot_examples(original_fname: List[str], original_kr_dialogue: List[str], exp_fname: List[str], exp_kr_dialogue: List[str], exp_en_summary: List[str], exp_kr_summary: List[str]) -> str:
    formatted_examples = []
    for i in range(len(exp_fname)):
        example = f"""Example {exp_fname[i]}:
